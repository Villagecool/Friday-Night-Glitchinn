...
function createMenu()
	...
	makeLuaSprite('simpi', 'menu/simpi', 0, 1015)
	makeLuaSprite('simpi2', 'menu/simpi-but-sus', 0, 0)
	makeLuaSprite('info', 'menu/info', 1325, 0)
	makeLuaSprite('bg', 'menu/simpybg', 0, 0)
...
function onUpdate(elapsed)
	if not allowCountdown and (keyReleased('space') or getPropertyFromClass('flixel.FlxG', 'keys.justReleased.ENTER')) and not startedCountdown and not canContinue then
		createMenu()
	end
	if not allowCountdown and (keyReleased('space') or getPropertyFromClass('flixel.FlxG', 'keys.justReleased.ENTER')) and not startedCountdown and canContinue then
		allowCountdown = true
		esc = false
		startCountdown()
	...
	if getMouseX('hud') > getProperty('simpi.x') and getMouseY('hud') > getProperty('simpi.y') and getMouseX('hud') < getProperty('simpi.x') + getProperty('simpi.width') and getMouseY('hud') < getProperty('simpi.y') + getProperty('simpi.height') and curStep < 0 then
		MR = true
